...
- **Never create a custom field for**: `email`, `phone`, `linkedInUrl`, `education`, `address`, `notes`, `careerGoal`, `preferredRegions`, `preferredLocationNotes`
...
- **Never pass `_region_names`**: That field no longer exists. Use `preferredLocationNotes` for free-text region display and `workRegionIds` for structured region subscriptions.
